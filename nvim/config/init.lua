...
-- Terminal
vim.keymap.set("n", "<leader>tt", "<cmd>split | term<CR>", { desc = "[T]erminal horizontal split" })
vim.keymap.set("n", "<leader>tv", "<cmd>vsplit | term<CR>", { desc = "[T]erminal vertical split" })
...
require("lazy").setup({
    ...
    },
    {
        ...
        config = function()
            ...
            local servers = {
                -- clangd = {},
                gopls = {},
                -- pyright stays enabled for go-to-definition/hover/completion, but its own
                -- type-checking diagnostics are turned off: mypy (via nvim-lint below) owns types.
                pyright = {
                    settings = {
                        pyright = {
                            disableOrganizeImports = true, -- ruff owns import organizing
                        },
                        python = {
                            analysis = {
            ...
            -- vim.lsp.enable(), bypassing any `handlers` table passed to its setup()
            -- below — so per-server overrides (capabilities, settings) have to be
            -- registered directly with vim.lsp.config() first; automatic_enable then
            -- picks up whatever's registered here when it calls vim.lsp.enable().
            ...
            require("mason-lspconfig").setup({
                ...
                -- rust_analyzer is excluded here — rustaceanvim attaches it instead.
    ...
    -- Active theme: Kanagawa Wave
    -- Switch anytime with <leader>tc (live preview) or :colorscheme <name>
    -- Available: rose-pine, rose-pine-moon, rose-pine-dawn,
    --            catppuccin, catppuccin-mocha, catppuccin-macchiato,
    --            kanagawa, kanagawa-wave, kanagawa-dragon, kanagawa-lotus,
    --            night-owl, ayu-dark, ayu-mirage, ayu-light
    {
        "rose-pine/neovim",
        name = "rose-pine",
        lazy = true,
    ...
    { -- Collection of various small independent plugins/modules
        ...
        config = function()
            ...
            -- Animated indent scope line (highlights current block)
    ...
    -- Buffer tabs (visual overview of open files, like editor tabs)
    {
    ...
    -- Quick file switching (mark files, jump to them instantly)
    {
